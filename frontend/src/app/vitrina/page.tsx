      ...
      <header className="bg-white border-b border-black/[0.06]">
        <div className="max-w-[1200px] mx-auto px-4 py-8">
          <nav className="flex items-center gap-2 text-ink/40 text-xs mb-3">
            ...
            <span>/</span>
            <span className="text-ink/70">Vitrina</span>
          </nav>
        ...
        <aside className="lg:w-52 shrink-0">
          <div className="bg-white rounded-2xl border border-black/[0.06] p-5 sticky top-20">
            ...
            <div className="flex flex-col gap-0.5">
              <Link href="/vitrina" className={`text-sm py-1.5 px-2.5 rounded-lg transition-colors ${!cat ? "bg-primary text-white" : "text-sec hover:bg-black/5 hover:text-ink"}`}>
                Todas
              </Link>
              {categorias.map(c => (
                <Link key={c.slug}
                  href={`/vitrina?${new URLSearchParams({ cat: c.slug, ...(cid && { cidade: cid }) }).toString()}`}
            ...
            <div className="flex flex-col gap-0.5">
              <Link href={`/vitrina${cat ? `?cat=${cat}` : ""}`} className={`text-sm py-1.5 px-2.5 rounded-lg transition-colors ${!cid ? "bg-primary text-white" : "text-sec hover:bg-black/5 hover:text-ink"}`}>
                Todas
              </Link>
              {CIDADES.map(c => {
                ...
                return (
                  <Link key={slug}
                    href={`/vitrina?${new URLSearchParams({ ...(cat && { cat }), cidade: slug }).toString()}`}
        ...
        <section className="flex-1 min-w-0">
          {negocios.length === 0 ? (
            <div className="bg-white rounded-2xl border border-black/[0.06] p-14 text-center">
              ...
              <Link href="/vitrina" className="mt-4 inline-block text-sm font-semibold text-primary hover:text-accent transition-colors">
                Ver todos →
              </Link>
            </div>
          ) : (
