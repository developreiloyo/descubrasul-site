    ...
    <main className="mx-auto max-w-4xl px-4 py-12">
      ...
      <section className="mt-8 flex flex-col gap-4">
        ...
        {produtos.map((p, index) => (
          <div key={p.id} className={`rounded-xl border bg-white p-4 ${index === 0 ? "border-amber-400 ring-1 ring-amber-400/30" : "border-ink/10"}`}>
            ...
            <div className="mt-3 border-t border-ink/10 pt-3">
              ...
              <div className="flex gap-2 flex-wrap">
                {p.fotos?.map((f) => (
                  <div key={f.id} className="relative size-16 overflow-hidden rounded-lg border border-ink/10">
                    <img
                      src={mediaUrl(f.foto) || ""}
                ...
                {(p.fotos?.length || 0) < 3 && (
                  <label className="size-16 rounded-lg border-2 border-dashed border-ink/20 flex flex-col items-center justify-center cursor-pointer hover:border-primary transition">
