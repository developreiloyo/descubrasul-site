...
export default function MetricasPage() {
  ...
  if (carregando) {
    return (
      <main className="mx-auto max-w-5xl px-4 py-12">
        <p className="text-ink/50">Carregando métricas...</p>
      </main>
    );
  }
  ...
  if (!dados?.is_pro) {
    return (
      <main className="mx-auto max-w-5xl px-4 py-12">
        <h1 className="text-3xl font-bold">Minhas Métricas</h1>
        <div className="mt-8 rounded-2xl border-2 border-dashed border-secondary/40 bg-secondary/5 p-8 text-center">
          <p className="text-4xl">📊</p>
          <h2 className="mt-4 text-xl font-bold">
            Descubra como seu negocio esta performando
          </h2>
          <p className="mt-2 text-ink/60 max-w-md mx-auto">
            Com o Plano Pro voce ve quantas pessoas visitaram sua pagina,
          ...
          <div className="mt-6 grid grid-cols-3 gap-4 max-w-sm mx-auto opacity-40 select-none">
            <div className="rounded-xl bg-white border p-4">
              ...
              <p className="text-xs text-ink/60 mt-1">Visualizacoes</p>
            </div>
            <div className="rounded-xl bg-white border p-4">
            ...
            <div className="rounded-xl bg-white border p-4">
              ...
              <p className="text-xs text-ink/60 mt-1">Conversao</p>
            </div>
          </div>
          <Link
            href="/painel/planos"
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-12">
      <h1 className="text-3xl font-bold">Minhas Métricas</h1>
      <p className="mt-1 text-ink/50 text-sm">Últimos 30 dias · {dados.plano_display}</p>

      {/* ─── Cards de resumo ────────────────────────────── */}
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="rounded-xl border border-ink/10 bg-white p-5">
          <p className="text-sm text-ink/50">Visualizacoes</p>
          <p className="mt-1 text-3xl font-bold text-primary">
            {resumo?.total_views.toLocaleString("pt-BR")}
          </p>
        </div>
        <div className="rounded-xl border border-ink/10 bg-white p-5">
          <p className="text-sm text-ink/50">Contatos WhatsApp</p>
          <p className="mt-1 text-3xl font-bold text-green-600">
            {resumo?.total_whatsapp.toLocaleString("pt-BR")}
          </p>
        </div>
        <div className="rounded-xl border border-ink/10 bg-white p-5">
          <p className="text-sm text-ink/50">Taxa de Conversao</p>
          <p className="mt-1 text-3xl font-bold text-secondary">
            {resumo?.taxa_conversao}%
          </p>
        </div>
        <div className="rounded-xl border border-ink/10 bg-white p-5">
          <p className="text-sm text-ink/50">Compartilhamentos</p>
      ...
      {grafico && grafico.length > 0 && (
        <section className="mt-8 rounded-xl border border-ink/10 bg-white p-6">
          <h2 className="mb-4 text-lg font-semibold">Evolucao dos ultimos 30 dias</h2>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={grafico.map((p) => ({
              ...p,
      ...
      {origens && origens.length > 0 && (
        <section className="mt-6 rounded-xl border border-ink/10 bg-white p-6">
          <h2 className="mb-4 text-lg font-semibold">De onde vem seus visitantes</h2>
              ...
              <div key={o.nome} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-ink/50">{o.valor} visitas · {o.pct}%</span>
                  </div>
      ...
      {tabela && tabela.length > 0 && (
        <section className="mt-6 rounded-xl border border-ink/10 bg-white p-6">
          <h2 className="mb-4 text-lg font-semibold">Ultimos 7 dias</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ink/10 text-left text-ink/50">
                  ...
                  <th className="pb-2">Conversao</th>
                </tr>
              </thead>
              <tbody>
                {tabela.map((linha) => (
